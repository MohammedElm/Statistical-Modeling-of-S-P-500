...
class MHP:
    ...
    def EM(self, Ahat, mhat, omega, seq=[], smx=None, tmx=None, regularize=False, 
        ...
        N = len(seq)
        dim = mhat.shape[0]
        Tm = float(np.array(seq)[-1,0]) if Tm < 0 else float(Tm)
        sequ = np.array(seq)[:,1].astype(int)
        ...
        diffs = pairwise_distances(np.array([np.array(seq)[:,0]]).T, metric = 'euclidean')
        ...
        def sum_pij(a,b):
            c = cartesian([np.where(np.array(seq)[:,1]==int(a))[0], np.where(np.array(seq)[:,1]==int(b))[0]])
        ...
        while k < maxiter:
            ...
            mhat = np.array([np.sum(p_ii[np.where(np.array(seq)[:,1]==i)]) \
            ...
            if k % 10 == 0:
                ...
                term3 = np.sum(np.sum(Ahat[u,int(np.array(seq)[j,1])] for j in range(N)) for u in range(dim))
    ...
    def get_integral(self,ct,d=0):
        seq = np.array(self.data)
        if not np.all(ct > seq[:,0]): seq = seq[seq[:,0] < ct]
        return self.mu[d] + \
            -self.alpha[0]*np.sum([np.exp(-self.omega*(ct-t))-1 for t,j in seq])+self.mu[d]*ct
            
    def get_residuals(self):
        residuals=[self.get_integral(e[0])[0] for e in self.data]
        return [residuals[i]-residuals[i-1] for i in range(1,len(residuals))]

    def plot_rates(self, horizon=-1):
        if horizon < 0:
            horizon = np.amax(np.array(self.data)[:,0])
        ...
        for i in range(self.dim):
            ...
            # plot events
            subseq = np.array(self.data)[np.array(self.data)[:,1]==i][:,0]
        ...
        plt.tight_layout()
        #plt.savefig("rates_event_2011_2019")
